...
export default function Results() {
  ...
  const { data: sourceFeature } = useGetFeature(prd?.featureId ?? 0, {
    query: { enabled: !!prd?.featureId },
  });
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              if (!prd) return;
              const title   = encodeURIComponent(prd.featureTitle);
              const desc    = encodeURIComponent(sourceFeature?.description ?? "");
              const owner   = encodeURIComponent(sourceFeature?.ownerName ?? "");
              navigate(`/?title=${title}&desc=${desc}&owner=${owner}`);
            }}
            title="Pre-fill the generator with these inputs"
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all bg-muted/40 text-muted-foreground border border-card-border hover:bg-muted/70 hover:text-foreground"
          >
